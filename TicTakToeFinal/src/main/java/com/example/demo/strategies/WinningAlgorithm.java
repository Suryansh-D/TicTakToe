...
public class WinningAlgorithm {
    ...
    public boolean checkWinner(Board board, Move move) {
        ...
        if (currRowMap.get(character) == board.getSize(row)) {
            return true;
        }
        ...
        if (currColMap.get(character) == board.getSize(col)) {
            return true;
        }
        ...
        if (row == col) {
            ...
            if (leftDiagonalMap.get(character) == board.getSize(row)) {
                return true;
            }
        }

        //Right Diagonal
        if (row + col == board.getSize(row) - 1) {
            ...
            if (rightDiagonalMap.get(character) == board.getSize(row)) {
                return true;
            }
        }
